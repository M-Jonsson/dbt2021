...
class Bead_protocol_config():
    ...
    def call_checkbox_beads(self):
        ...
        if int(self.entry_sample_no.get()) < 8:
            Checkbox('dna8', self.window)
        else:
            Checkbox('dna96', self.window)
...
class Tube_rack_base():
    ...
    def __init__(self, parent):
        ...
        self.test_label = ttk.Label(text='testing', master=self.frame)
...
class Checkbox:
    ...
    def __init__(self, protocol_type, parent):
        # self.window = tk.Toplevel()
        ...
        if protocol_type == 'qpcr':
            self.protocol_type = 'qpcr'
            self.image_name = 'qPCR\\test.gif'
            self.pipette_text = '\n     Left: ? ?-channel\n     Right: ? ?-channel'
            # self.tube_rack_layout = Tube_rack_grid(self.frame)
            # qPCR_protocol_config.fill_notebook(self.parent)
            # self.tube_rack_base = Tube_rack_base(self.parent)
            # self.tube_rack_base.fill_notebook()

        elif protocol_type == 'dna96':
            self.protocol_type = 'dna'
            self.image_name = 'ui\\deck_96.gif'
            self.pipette_text = '\n     Left: P10 8-channel\n     Right: P300 8-channel'
            self.image = Checkbox_img(self.frame, self.image_name)

        elif protocol_type == 'dna8':
            self.protocol_type = 'dna'
        ...
        # self.image = tk.PhotoImage(file=self.image_name)
